...
@File: label_xy_to_bbox.py
@Time: 2020-03-05 14:54
@Last_update: 2020-03-05 14:54
@Desc: None
@==============================================@
@      _____             _   _     _   _       @
...
def label_xy_to_bbox(label_array, bbox_name_list=['xmin','ymin','xmax','ymax']):
    """
    把分割的单层二值图像转换为bbox
    ...
    for i in np.unique(labeled_array):
        ...
        # 3. 找到bbox的数值
        x_pos, y_pos = np.where(labeled_array == i)
        xmin = x_pos.min()
        xmax = x_pos.max()
        ymin = y_pos.min()
        ymax = y_pos.max()
